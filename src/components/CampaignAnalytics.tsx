...
export function CampaignAnalytics({ campaignId }: CampaignAnalyticsProps) {
  ...
  const fetchAnalytics = async () => {
    try {
      ...
      const emailsSent = conversationData?.filter(c => c.channel === 'email' && c.from_role === 'ai').length || 0;
      ...
      for (let i = daysToShow - 1; i >= 0; i--) {
        ...
        dailyActivity.push({
          ...
          emails: dayConversations.filter(c => c.channel === 'email').length,
        });
      }
                    ...
                    <div key={index} className="flex flex-col items-center space-y-2 flex-1 max-w-16">
                      ...
                      <div className="relative flex flex-col justify-end h-48 w-8">
                        ...
                        {day.emails > 0 && (
                          <div
                            className={`w-full rounded-b-sm ${
                              theme === 'gold' ? 'bg-pink-400' : 'bg-indigo-500'
                            } transition-all duration-300 hover:opacity-80`}
                            style={{
                        ...
                        {/* Empty state */}
                        {(day.calls + day.sms + day.whatsapp + day.emails) === 0 && (
                        ...
                        {(day.calls + day.sms + day.whatsapp + day.emails) > 0 && (
                          <div className={`absolute -top-6 left-1/2 transform -translate-x-1/2 text-xs font-medium ${
                            ...
                            {day.calls + day.sms + day.whatsapp + day.emails}
                          </div>
                        )}
                      </div>
              ...
              <svg className="w-full h-full" viewBox="0 0 800 200">
                ...
                {analytics.dailyActivity.length > 1 && (
                  <>
                    ...
                    <polyline
                      fill="none"
                      stroke={theme === 'gold' ? '#ec4899' : '#6366f1'}
                      strokeWidth="3"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      points={analytics.dailyActivity.map((day, index) => {
                        const x = 40 + (index * (720 / (analytics.dailyActivity.length - 1)));
                        const y = 168 - ((day.emails / maxDailyValue) * 128);
                        return `${x},${y}`;
                      }).join(' ')}
                    />
                      ...
                      return (
                        <g key={index}>
                          ...
                          {day.emails > 0 && (
                            <circle
                              cx={x}
                              cy={168 - ((day.emails / maxDailyValue) * 128)}
                              r="4"
                              fill={theme === 'gold' ? '#ec4899' : '#6366f1'}
                              className="hover:r-6 transition-all cursor-pointer"
                            >
                              <title>{`${day.emails} Emails`}</title>
                            </circle>
                          )}
                        </g>
                      );
